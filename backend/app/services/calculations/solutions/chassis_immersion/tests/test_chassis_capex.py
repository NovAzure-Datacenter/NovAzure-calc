...
@pytest.mark.parametrize(
    "country,base_year,capacity_mw,expected_capex",
    [
...
def test_calculate_chassis_solution_capex_with_markup(
    country, base_year, capacity_mw, expected_capex
):
    result = calculate_chassis_solution_capex_with_markup(
        base_year, capacity_mw, country
    )
    assert result == expected_capex


@pytest.mark.parametrize(
    "input_data,expected_cooling_capex,expected_it_capex",
    [
        (
            {
...
def test_calculate_cooling_capex_without_it(
    input_data, expected_cooling_capex, expected_it_capex
):
...
def test_calculate_cooling_capex_with_it(input_data):
    ...
    assert (
        result["total_capex"]
        == result["cooling_equipment_capex"] + result["it_equipment_capex"]
    )
