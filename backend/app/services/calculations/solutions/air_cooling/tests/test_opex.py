...
from app.services.calculations.solutions.air_cooling.opex import (
    calculate_annual_opex,
    calculate_total_opex_over_lifetime,
)
...
async def test_total_opex_lifetime_greater_than_annual():
    ...
    lifetime_result = await calculate_total_opex_over_lifetime(
        input_data_lifetime, 1000000.0
    )

    # Total lifetime OPEX should be significantly larger than annual OPEX
    assert (
        lifetime_result["total_opex_over_lifetime"] > annual_result["annual_opex"] * 5
    )
