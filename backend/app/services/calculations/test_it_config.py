...
@pytest.mark.parametrize(
    "channels_per_cpu,cpu_count,expected",
    [
        (8, 1, 8),
        (12, 2, 24),
        (16, 2, 32),
...
def test_calculate_total_memory_cost_per_server_defaults():
    ...
    expected_channels = (
        DEFAULT_DDRS_6400_CHANNELS * DEFAULT_NUMBER_OF_SOCKETS_PER_SERVER
    )
    expected = (
        expected_channels * DEFAULT_DIMM_CAPACITY_IN_GB * DEFAULT_DIMM_COST_PER_GB
    )
    assert result == expected


@pytest.mark.parametrize(
    "total_channels,memory_per_channel,cost_per_gb,expected",
    [
...
def test_calculate_total_memory_cost_per_server_custom(
    total_channels, memory_per_channel, cost_per_gb, expected
):
    """Test memory cost calculation with custom values."""
    result = calculate_total_memory_cost_per_server(
        total_channels, memory_per_channel, cost_per_gb
    )
    assert result == expected
...
@pytest.mark.parametrize(
    "switch_cost,switches_per_rack,expected",
    [
        (8000, 2, 16000),
        (10000, 3, 30000),
        (6000, 2, 12000),
...
@pytest.mark.parametrize(
    "cpu_cost,memory_cost,network_cost,bom_cost,expected",
    [
...
def test_calculate_total_it_cost_per_server_custom(
    cpu_cost, memory_cost, network_cost, bom_cost, expected
):
    """Test total IT cost calculation with custom values."""
    result = calculate_total_it_cost_per_server(
        cpu_cost, memory_cost, network_cost, bom_cost
    )
    assert result == expected


@pytest.mark.parametrize(
    "data_center_type,expected_type",
    [
...
@pytest.mark.parametrize(
    "rack_capacity_kw,data_center_type,expected",
    [
...
def test_calculate_maximum_number_of_chassis_per_rack_for_air_valid(
    rack_capacity_kw, data_center_type, expected
):
    """Test rack capacity calculation for valid data center types."""
    result = calculate_maximum_number_of_chassis_per_rack_for_air(
        rack_capacity_kw, data_center_type
    )
    assert result == expected
...
def test_calculate_maximum_number_of_chassis_per_rack_for_air_invalid(
    rack_capacity_kw, data_center_type, expected
):
...
@pytest.mark.parametrize(
    "chassis_count,data_center_type,expected",
    [
...
def test_calculate_total_air_power_kw_per_rack_valid(
    chassis_count, data_center_type, expected
):
...
@pytest.mark.parametrize(
    "chassis_count,data_center_type",
    [
...
def test_rack_capacity_integration():
    ...
    max_servers = calculate_maximum_number_of_chassis_per_rack_for_air(
        rack_capacity_kw, data_center_type
    )
