...
async def test_advanced_mode_without_it_cost(reset_inputs):
    ...
    assert result['air_cooling']['total_capex'] == result['air_cooling']['cooling_equipment_capex']
    assert result['chassis_immersion']['total_capex'] == result['chassis_immersion']['cooling_equipment_capex']
...
async def test_advanced_mode_with_it_cost(reset_inputs):
    ...
    assert result['air_cooling']['it_equipment_capex'] == result['chassis_immersion']['it_equipment_capex']
    
    # Total CAPEX should include IT costs
    expected_air_total = result['air_cooling']['cooling_equipment_capex'] + result['air_cooling']['it_equipment_capex']
    expected_chassis_total = result['chassis_immersion']['cooling_equipment_capex'] + result['chassis_immersion']['it_equipment_capex']
...
@pytest.mark.asyncio
@pytest.mark.parametrize("country,capacity", [
    ("United States", 1.0),
    ("Singapore", 2.0),
    ("United Kingdom", 0.5),
...
async def test_different_countries_and_capacities(reset_inputs, country, capacity):
    ...
    assert result['chassis_immersion']['cooling_equipment_capex'] > result['air_cooling']['cooling_equipment_capex']
...
async def test_total_cost_of_ownership_calculation(reset_inputs):
    ...
    expected_air_tco = air_cooling['total_capex'] + air_cooling['total_opex_over_lifetime']['total_opex_over_lifetime']
    ...
    assert abs(chassis_immersion['total_cost_of_ownership'] - expected_chassis_tco) < 0.01
...
def test_update_inputs_functionality(reset_inputs):
    ...
    assert air_rack_cooling_capacity_kw_per_rack['air_rack_cooling_capacity_kw_per_rack'] == 20
    
    import app.services.calculations.main as main_module
    assert main_module.advanced == True
...
async def test_hpc_ai_vs_general_purpose(reset_inputs):
    ...
    assert hpc_result['air_cooling']['it_equipment_capex'] > gp_result['air_cooling']['it_equipment_capex']
    assert hpc_result['chassis_immersion']['it_equipment_capex'] > gp_result['chassis_immersion']['it_equipment_capex']
    assert hpc_result['air_cooling']['total_capex'] > gp_result['air_cooling']['total_capex']
    assert hpc_result['chassis_immersion']['total_capex'] > gp_result['chassis_immersion']['total_capex']
    
    # Cooling equipment CAPEX should be the same for both
    assert hpc_result['air_cooling']['cooling_equipment_capex'] == gp_result['air_cooling']['cooling_equipment_capex']
    assert hpc_result['chassis_immersion']['cooling_equipment_capex'] == gp_result['chassis_immersion']['cooling_equipment_capex']
