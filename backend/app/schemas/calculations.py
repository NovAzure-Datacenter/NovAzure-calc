...
class CoolingSolutionRequest(BaseModel):
    ...
    percentage_of_utilisation: float = Field(..., description="Percentage of utilisation")
    ...
    air_rack_cooling_capacity_kw_per_rack: Optional[float] = Field(default=None, description="Air rack cooling capacity per rack in kW")
    data_center_type: Optional[str] = Field(default=None, description="Data center type")

    # Advanced Data Centre Configuration inputs
    inlet_temperature: float = Field(default=27, description="Inlet temperature")
    electricity_price_per_kwh: float = Field(default=0.1, description="Electricity price per kWh")
    water_price_per_litre: float = Field(default=0.0001, description="Water price per litre")
...
class SingleCoolingSolution(BaseModel):
    ...
    total_opex_over_lifetime: int = Field(..., description="Total OPEX over planned lifetime")
    tco_excluding_it: int = Field(..., description="Total cost of ownership excluding IT")
    tco_including_it: int = Field(..., description="Total cost of ownership including IT")


class CoolingSolutionResult(BaseModel):
    air_cooling_solution: SingleCoolingSolution = Field(..., description="Air cooling solution")
    chassis_immersion_solution: SingleCoolingSolution = Field(..., description="Chassis immersion solution")
