			...
			<div className="max-h-[55vh] overflow-y-auto">
				<TooltipProvider>
					<Table>
						...
						<TableBody>
							{/* Add Calculation Row - Now at the top */}
							{isAddingCalculation && (
								<TableRow className="bg-blue-50 border-2 border-blue-200 shadow-md">
									...
									{isAddFormulaExpanded ? (
										...
										/* Normal add calculation columns (not expanded) */
										<>
											{/* Name */}
											<TableCell>
												<Input
											...
											<div className="flex flex-wrap gap-2">
												{(params as any[]).map((param: any) => (
													<button
														...
														type="button"
														onClick={() => insertIntoFormula(param.name)}
														className="h-6 px-3 text-xs border rounded hover:bg-muted transition-colors"
														title={`${param.name}: ${param.description}`}
													>
														{param.name}
													</button>
