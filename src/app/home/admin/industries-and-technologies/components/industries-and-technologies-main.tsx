...
import { TableView } from "./industry-list-view";
import { GridView } from "./industry-grid-view";
import { TechnologyList } from "./technology-list-view";
import { CreateIndustryDialog } from "./create-industry-dialog";
import { CreateTechnologyDialog } from "./create-technology-dialog";
import type { Industry, Technology } from "../types";
import { TechnologyGridView } from "./technology-grid-view";
import {
	getIndustries,
	createIndustry,
	deleteIndustry,
} from "@/lib/actions/industry/industry";
import {
	getTechnologies,
	createTechnology,
	deleteTechnology,
...
export function IndustriesAndTechnologiesMain() {
	...
	const handleCreateIndustry = async (
		...
		try {
			...
			if (result.error) {
				...
				setIndustries((prev) =>
					prev.filter((i) => i.id !== optimisticIndustry.id)
				);
			...
			setIndustries((prev) =>
				prev.filter((i) => i.id !== `temp-${Date.now()}`)
			);
		...
		try {
			...
			if (result.error) {
				...
				setTechnologies((prev) =>
					prev.filter((t) => t.id !== optimisticTechnology.id)
				);
				toast.error(result.error);
				return;
			}
			...
			// Revert optimistic update
			setTechnologies((prev) =>
				prev.filter((t) => t.id !== `temp-${Date.now()}`)
			);
					...
					<div className="w-full">
						{viewMode === "table" ? (
							<TableView
								...
								onIndustryDeleted={handleIndustryDeleted}
							/>
						) : (
							<GridView
								data={filteredIndustries}
