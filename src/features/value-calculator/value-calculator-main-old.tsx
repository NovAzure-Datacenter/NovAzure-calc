 ...
 */
export default function ValueCalculatorMainOld() {
	const { user } = useUser();
	 ...
	 */
	const fetchClientSolutions = useCallback(
		async (industryId: string, technologyId: string) => {
	 ...
	 */
	const fetchSolutionTypes = useCallback(
		async (industryId: string, technologyId: string) => {
 ...
 */
function TabNavigation({ hasCalculated, comparisonMode }: TabNavigationProps) {
	return (
